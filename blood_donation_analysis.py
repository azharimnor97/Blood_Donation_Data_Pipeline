...
def daily_trend_facility_viz(df):
    ...
    ax = daily_trends_facility_viz[[previous_update,latest_update]].plot.bar(fontsize=8.0, xlabel="Blood Donation Facility", ylabel="Number of blood donation", figsize=(15,5), stacked=False, title=F"Latest daily trend of blood donation at each facility in Malaysia")
    buf = io.BytesIO()
...
def trend_year_data():
    try:
        with session_scope() as session:
            query = f"SELECT `date`,`hospital`,`daily` FROM `donation_facility` WHERE `date` >= '2013-01-01' AND `date` < '2024-01-01' ORDER BY `donation_facility`.`date`,`donation_facility`.`hospital` ASC;"
            df = pd.read_sql_query(query, con=session.bind)
            ...
            df_year_viz = df_year_pivot.plot(figsize=(15,30), subplots=True, layout=(11,2), xticks=x_ticks, fontsize=8.0, rot=45.0)
...
def retention_rate(df1,df2,year): #(current year, previous year)
    ...
    retention_rate = (returning_donor/total_donor_previous) * 100
    retention_rate = "{:.2f}".format(retention_rate)
    message = f"The blood donor retention rate for the {year} is {retention_rate} %. The number of returning blood donor in {year} is {returning_donor}, compared to {total_donor_previous} total number of blood donor in the previous year"
    #print(f"The blood donor retention rate for the {year} is {retention_rate} %. The number of returning blood donor in {year} is {returning_donor}, compared to {total_donor_previous} total number of blood donor in the previous year")
    return message